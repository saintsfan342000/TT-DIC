...
FIRST GENERATION Python DIC analysis code.
-From time when STF.dat was made via separate script
-Not vectorized

Designed to be the complete TT analysis code.
Will do the same analysis as Nico's code
Will compute rotation and delta/L
Will log the max point, and the stages it occurs in
Will log the 2nd max point, and the stages it occurs in
Will log the yield stress
'''
...
FS = 32
SS = 8
arampath = r'F:\TT_New\TT2-12_FS32SS8\AramisExport_MissingRemoved'
prefix = 'TT2-12_FS32SS8_'
makecontours = True
BIN = True

key = read_excel('{}/../../TT-Summary.xlsx'.format(arampath),sheetname='Summary',header=None,index_col=None,skiprows=1).values
alpha, Rm,thickness = key[ key[:,0] == expt, [1,3,4]].flatten()

os.chdir('{}/..'.format(arampath))
...
if BIN == True:
    ...
    A = read_csv('{}/{}{:.0f}.dat'.format(arampath,prefix,last),sep=',').values
...
STF = n.loadtxt('{}/STF.dat'.format(arampath),delimiter=',')           
...
if not os.path.exists('prof_stages.dat'):
    p.figure()
    p.plot(STF[:,2])
    p.plot(LL,STF[LL,2],'ro',ms=10)
    p.title('Click where the first\ncalc stage will be', fontsize=30)
    p.axis([0,1.1*STF[-1,0],0,1.2*n.max(STF[:,2])])
    yldsts = n.asarray(p.ginput(1)).flatten()[1]
    p.close()

    # Make evenly spaced increments
    for i in range(len(STF[:,2])):
        if STF[i,2] >= yldsts:
            break

    #Two before the limit load, seven after
    x1 = int(n.around((LL - i)/3));
    x2 = int(n.around((len(STF[:,2])-1-LL)/7)-1)
    profStg = array([i, i+x1, LL, LL+x2, LL+2*x2, LL+3*x2, LL+4*x2, LL+5*x2, LL+6*x2, last])
else:
...
if not os.path.exists('disp-rot_limits.dat'):
    ...
    if BIN == True:
        ...
        A = read_csv('{}/{}{:.0f}.dat'.format(arampath,prefix,0),sep=',').values
    #[0]Index_x [1]Index_y [2,3,4]Undef_X,Y,Z inches [5,6,7]Def_X,Y,Z inches [8,9,10,11]DefGrad (11 12 21 22) *)
...
for k in range(last,-1,-1):
    ...
    if k == 0:
        ...
        if BIN == True:
            ...
            A = read_csv('{}/{}{:.0f}.dat'.format(arampath,prefix,k),sep=',').values
        #[0]Index_x [1]Index_y [2,3,4]Undef_X,Y,Z inches [5,6,7]Def_X,Y,Z inches [8,9,10,11]DefGrad (11 12 21 22) *)
...
os.system('python F:\TT_New\AA_PyFiles\TT2_DIC_Analysis_Plots.py {} {} {} {} {}/..'.format(int(expt), alpha, int(FS), int(SS), arampath))
