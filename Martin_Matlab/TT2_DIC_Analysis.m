...
%%%%%% PRELIMINARY DATA to ENTER %%%%%% 
    last = 350;         %Last stage in which specimen is not fractures
    TT2=31;             %Expt number
    alpha=0.25;          %Alpha
%Specimen measurements
    Rm = 0.8932;        % Mean radius
    thickness = 0.0372; %Wall thickness
    Lg = 0.4;         
    tube=16;
%DIC paramaters
    Facet_size = 19;    %pix                            
...
%path for the files that the PERL program produces
    PATH = 'E:\Martin_Experiments\AAA_TensionTorsion\TT2-31\AramisExport_MissingRemoved\'; %MUST CHANGE
%prefix of the name
...
%%%%%%%%%%%   Make the plot in which you'll draw a box %%%%%%%%%%%%%%%%%%%% 
    ...
    crop = ginput(2);
    %crop = [0.2      -3.1434;     0.085932      -2.6303]
    Xmin = min(crop(:,1));  Xmax = max(crop(:,1));  Ymin = min(crop(:,2)); Ymax = max(crop(:,2));
    %Xmin = Xmin * Rm; Xmax = Xmax * Rm; Ymin =Ymin*thickness;   Ymax=Ymax*thickness; 
...
for k = 1:last
    ...
    A = load(name);
    %(1)Index_x  (2)Index_y (3,4,5) Undef Coord (X,Y,Z) ;; (6,7,8) Deformed Coord (X,Y,Z) (9,10,11,12) DefGrad (11 12 21 22) *)
    ...
    export_max(k,:) =  [length(passed), STF(k,[2,3,4]),NEx_pass(index_max),NEy_pass(index_max),abs(gamma_pass(index_max)),LEp_pass(index_max)];
    export_mean(k,:) = [length(passed), STF(k,[2,3,4]),nanmean(NEx_pass),nanmean(NEy_pass),abs(nanmean(gamma_pass)),nanmean(LEp_pass)];
    export_stdv(k,:) = [length(passed), STF(k,[2,3,4]),nanstd(NEx_pass),nanstd(NEy_pass),abs(nanstd(gamma_pass)),nanstd(LEp_pass)];
...
%%%% Profile of ep for different data_force_points....Still in the loop for "k=1:last" %%%%
    if any(profStages==k) == 1       %Checks to see if the current stage being analyzed (k) is one of our profStages
        ...
        for j = 1:length(profColum) 
            ...
            profLEp{prof_count}(j,1) = A(profColum(j),7)/thickness;     
            F = [[A(profColum(j),9),A(profColum(j),10)] ;[A(profColum(j),11),A(profColum(j),12)]]; %transformation gradient F=RU
            U = transpose(F)*F;             %stretching tensor???
            diagU = eig(U);                 %principal stretch
...
%                 F = [[A(RowsInAInScotGrid(m),9),A(RowsInAInScotGrid(m),10)] ;[A(RowsInAInScotGrid(m),11),A(RowsInAInScotGrid(m),12)]];     %transformation gradient F=RU
%                 U = transpose(F)*F;                         %stretching tensor???
...
%     end   
%     export_Scott(k,:) = [nanmean(OurSize_Scott),STF(k,[2,3,4]),nanmean(NEx_Scott),nanmean(NEy_Scott),abs(nanmean(gamma_Scott)),nanmean(LEp_Scott)];
...
plot(export_mean(:,7),export_mean(:,6),'bx');       
hold on
plot(export_max(:,7),export_max(:,6),'ro');
%plot(export_Scott(:,7),export_Scott(:,6),'k+');
...
%legend('Mean','Max', 'Average Value (Scott Grid)' ,'Location','SouthWest')
legend('Mean','Max', 'Location','SouthWest')
...
errorbar(export_mean(:,2),export_mean(:,end),0.5*export_stdv(:,end));
hold on
plot(export_max(:,2),export_max(:,end),'ro')
%plot(export_Scott(:,2),export_Scott(:,end),'k+');
...
axis([0 max(export_max(:,2)) 0 1.1*max(export_max(:,end))])
...
fprintf(output,'%% nb of points [] Time [s] Axial Stress [ksi] Shear Stress Technical NEx [] NEy [] Gamma [rad] True cumulative strain [] \n');
fprintf(output, '%d %f %f %f %f %f %f %f\n', export_max');
fclose(output);
...
% fprintf(output,'%% size of the averaging zone for like-Scott [in] Time [s] Axial Stress Shear Stress [ksi] Technical NEx [] NEy [] Gamma [rad] True cumulative strain [] \n');
% fprintf(output, '%f %f %f %f %f %f %f %f\n', export_Scott');
% fclose(output);

output = fopen(sprintf('%s/box_limts.dat',folder),'w');    
