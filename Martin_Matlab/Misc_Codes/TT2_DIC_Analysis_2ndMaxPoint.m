function [AramIJ_max2,stg,LEp_second]=TT2_DIC_Analysis_2ndMaxPoint();
...
%Add other matlab functions
addpath(sprintf('%s\\Matlab\\extras',patha(1:2)));
...
for k = last
    ...
    LE_sorted=sort(LEp_pass,'descend');  %Sort LEp_pass in descending order
    LEp_second=LE_sorted(2);
    max2loc=find(LEp_pass==LEp_second);    %Now find the index in LE_pass of the second highest LEp
    AramIJ_max2=[aramI_pass(max2loc) aramJ_pass(max2loc)];
...
for k=1:last;
    ...
    name = sprintf('%s\\%s%d.dat',patha,prefix,k); %Name of the stage file to open
    A = load(name);
    good=find(A(:,1)==AramIJ_max2(1) & A(:,2)==AramIJ_max2(2));   %Find will return an empty matrix there aren't any matches
...
fid=fopen(sprintf('%s\\MaxPoint2.dat',pathb),'w');
fprintf(fid,sprintf('First Row: Aramis I,J index of 2nd-to-highest LEp point; LEp=%.3f vs max of %.3f\nSecond Row and beyond: (1)Stage in which point exists (2)Row in that AramisExport_MissingRemoved file where that point is\n',LEp_second,max(LEp_pass)));
fprintf(fid,'%.0f %.0f\n',AramIJ_max2');
fprintf(fid,'%.0f %.0f\n',stg');
fclose all
%}
