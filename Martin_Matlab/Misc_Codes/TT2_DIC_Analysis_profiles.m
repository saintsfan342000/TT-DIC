...
%%%%%% PRELIMINARY DATA to ENTER %%%%%% 
    TT2=15;             %Expt number
    last = 263;         %Last stage in which specimen is not fractures
    thickness = 0.0384625;
...
%     prefix = 'AramisExport_MissingRemoved_expanded\TT2-17_DC15_FS19_SS6_';                %MUST CHANGE don't include underscore after linear
     profMaxlocA = 149;   %Determined to be the i index of the max stn point in the last stage 84 is best so far
     prefix = 'AramisExport_MissingRemoved\TT2-15_DC15_FS19_SS6_';                %MUST CHANGE don't include underscore after linear
...
for k = [profStages]';
   ...
   for j = 1:length(profColum) 
        ...
        profLEp{prof_count}(j,1) = (A(profColum(j),4)+A(profColum(j),7))/thickness/25.4;     
