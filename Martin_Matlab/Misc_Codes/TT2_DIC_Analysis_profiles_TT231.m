...
%     prefix = 'AramisExport_MissingRemoved_expanded\TT2-17_DC15_FS19_SS6_';                %MUST CHANGE don't include underscore after linear
     profMaxlocA = 62;   %Determined to be the i index of the max stn point in the last stage 84 is best so far
     prefix = 'AramisExport_MissingRemoved\TT2-31_FS19SS6-';                %MUST CHANGE don't include underscore after linear
...
for k = [profStages]';
   ...
   for j = 1:length(profColum) 
        ...
        profLEp{prof_count}(j,1) = (A(profColum(j),7))/.0372;     
...
if savestuff == 1
     ...
     for i=1:length(profLEp);
         xlswrite(sprintf('%sTT2-%d_MatlabResults\\StrainProfiles_ColIndex%d.xlsx',PATH,TT2,profMaxlocA),profLEp{i},sprintf('Stage%d',profStages(i)));
     end;

     export_fig(sprintf('%sTT2-%d_MatlabResults\\StrainProfiles.png',PATH,TT2),'-r200')
