function [A,LEp,B,emax,loc]=post_matlab_3D_DIC_linear_new();
...
%path for the files that the PERL program produces
    PATH = 'F:\Martin_Experiments\TT2-26\AramisExport_MissingRemoved\'; %MUST CHANGE
%prefix of the name
    prefix = 'TT2-26_DC16_FS19_SS6_MASKED'; %MUST CHANGE don't include underscore after linear
%DIC paramaters
    Facet_size = 19;%pix                            %MUST CHANGE                               %%VERY IMPORTANT MUST CHANGE
    Step_size = 6;%pix                              %MUST CHANGE
% # of the last picture
    last = 460; %MUST CHANGE
    TT2=26;
    alpha=0.75;
    calctype='Linear';
%plotting parameters
...
%Specimen dimensions in in                      %MUST CHANGE
    Rm = 0.8937;                                    %MUST CHANGE
    th = 0.0383;                                    %MUST CHANGE
    Lg = 0.4;                                       %MUST CHANGE
