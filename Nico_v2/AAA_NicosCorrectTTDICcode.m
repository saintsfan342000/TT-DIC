%function [Xmin_crop,Xmax_crop,Ymin_crop,Ymax_crop]=post_matlab_3D_DIC_linear_v2(); <---- Original name
function AAA_NicosCorrectTTDICcode();

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This is the code that Nico wrote during his 2nd visit to analyze TT2 DIC output files.
% This is the "Official one" - in other words, data that this code produces is that which
% we'll be publishing
...
%path for the files that the PERL program produces
    PATH = 'F:\Martin_Experiments\TT2-17\AramisExport_MissingRemoved_expanded\'; %MUST CHANGE
%prefix of the name
    prefix = 'TT2-17_expanded_FS19SS6'; %MUST CHANGE don't include underscore after linear
%DIC paramaters
...
%Specimen dimensions in in                      %MUST CHANGE
    Rm = 0.8942;                                    %MUST CHANGE
    th = 0.0382;  
    Lg = 0.4;                                       %MUST CHANGE
     if alpha >= 3.25
        Measured_output = 'Hoop';
        Stress_normalized = 2 * pi  * Rm^2 * th;
        cal = 2; %1V = 2 kips.in
        displayed = '\tau [ksi]' ;
    else
...
folder = sprintf('TT2-%d_DC15_FS_%d_SS_%d',TT2,Facet_size,Step_size);
mkdir(folder);
folder2 = sprintf('TT2-%d_DC15_FS_%d_SS_%d\\profile\\',TT2,Facet_size,Step_size);
mkdir(folder2);
...
%Xmin_crop=-0.371807014419573;   %For comparing to my modified code
%Xmax_crop=0.327031253296962;
%Ymin_crop=0.019659302207930;
%Ymax_crop=0.060041973162229;
% Xmin_crop=  -0.158893144157042;
% Xmax_crop=  0;
% Ymin_crop=  0.023808206758029;
%  Ymax_crop=  0.055616474975456;
...
%Coordinate of the center of the facets in the undeformed configuration 
X=(A(:,3))*coef/th; %Undeformed x coordinate of points
Y=(A(:,4))*coef/th; %Undeformed y coordinate of points
Z=(A(:,5))*coef/th; %Undeformed z coordinate of points
 %Determination of the min distance between facet
%Start by giving min_disp an erroneously large value...we'll soon
%recusively find the true minimum dist btwn any two points 
 min_disp = 1000; 
%Size(X,1) is simply the length of X, which is the # of index pts in the final stage
%i will go from 2 to length(X)-1 in 30 equally spaced increments
for i = [2 : round(size(X,1)/30) : size(X,1)-1] 
    %j will cover all indices up to i except i itself
    for j= [1:1:i-1,i+1:1:size(X,1)]
        %recursively find min_disp
        min_disp = min ( min_disp , ( (X(i)-X(j))^2 + (Y(i)-Y(j) )^2 + ( Z(i)-Z(j) )^2 )^0.5 );
    end
end
...
for k = 1 : last
    ...
    %Find the index i that are used to calculate the strain ie without index j missing
           for l = unique(A(scan,1))'
            %Column then tells us which matlab rows of the vector A(scan,1) contain this particular l
        ...
        %XcolenN(k)=length(x_undef_kept);    %Used for diagnosing Nico's error
        %XcosizeN{k}=size(x_undef_kept);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%        
%Calculation of the max equivalent strain and so on using a filtering method based on the strain ratio           
    ratio = NEy_kept ./ gamma_kept;
...
%over an area equivalent of the one of Scott (grid method)
    for i = size(x_undef_kept)      %%ERROR!!!  size(x_undef_kept) will return an array [#rows #columns] of the row vector x_undef_kept. Thus i doesn't iterate - it only takes a value of 1 (the # of rows) then whatever the length of x_undef_kept is.
       clear av_index;
       clear F;
       clear U;
       clear RotU;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        ylabel('undeformed y [in] (axial)','Fontsize',fts)
        zlabel('e^p','Fontsize',fts)
        mytitlestring=sprintf('Stage %d out of %d',k,last);
        title(mytitlestring,'FontSize',14)
        axis([min(x) max(x) min(y) max(y) min(LEp) max(LEp) min(LEp) max(LEp)])
        colorbar('location','EastOutside','Fontsize',fts) 
        set (gca,'Fontsize',ftstics)
...
%mytitlestring=sprintf('TT2-%d - \\alpha=%.2f - FS%d SS%d - %s (Deviation Check OFF)',TT2,alpha,Facet_size,Step_size,calctype)
...
%Max and mean values that satisfy ratio criteria are calc'd in line 277
%}